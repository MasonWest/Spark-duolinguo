# -*- coding: utf-8 -*-
"""生成 Level 4 三段文案（上一课回顾 / 本课要解决的问题 / 下一课伏笔）的静态预览页。
...
用法：python preview_level4_narrative.py
"""
import html
import json
import re
...
DB = "spark_quest.db"
OUT = "_l4_narrative_preview.html"
...
def main():
    ...
    cur = conn.cursor()
    cur.execute(
        "select title, order_index, content from lessons "
        "where level_id = (select id from course_levels where order_index = 4) "
    ...
    cards = []
    for title, oi, raw in rows:
        d = json.loads(raw)
        cards.append(f"""
    <section class="lesson">
      <h2 class="lt">L4-{oi + 1} · {html.escape(title)}</h2>
      <div class="grid">
...
<style>
  * {{ box-sizing: border-box; }}
  ...
  <h1>Level 4 · 「上一课回顾 / 本课要解决的问题 / 下一课伏笔」预览</h1>
